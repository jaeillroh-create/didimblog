...
-- 2. prompt_templates 시드 데이터 보장 (테이블이 있을 때만 실행)
DO $$
BEGIN
  -- prompt_templates 테이블이 존재하는지 확인
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'prompt_templates') THEN
    -- name 유니크 인덱스 추가
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE tablename = 'prompt_templates' AND indexname = 'prompt_templates_name_unique') THEN
      CREATE UNIQUE INDEX prompt_templates_name_unique ON public.prompt_templates (name);
    END IF;
...
--- 초안 끝 ---',
    ...
    RAISE NOTICE 'prompt_templates 시드 데이터 삽입 완료';
  ELSE
    RAISE NOTICE 'prompt_templates 테이블이 없습니다. 002_ai_engine.sql을 먼저 실행해주세요.';
  END IF;
END $$;

...
/**
 * LLM 초안 생성 실행 — runGeneration 래퍼 (Server Action)
 * 주요 실행 경로는 /api/generate Route Handler (스트리밍)
 * 이 함수는 폴백/호환용으로 유지
 */
export async function executeGeneration(
  generationId: number
): Promise<{ success: boolean; error?: string }> {
  const { runGeneration } = await import("@/lib/generation-runner");
  return runGeneration(generationId);
}

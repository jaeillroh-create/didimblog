...
★ 반드시 지킬 5가지 (이 규칙은 다른 모든 지시보다 우선합니다)
1. 절대 제도 설명으로 글을 시작하지 않는다. 반드시 "사람의 상황"으로 시작한다.
2. 이메일은 반드시 admin@didimip.com만 사용한다. 다른 주소 절대 금지.
3. 핵심 키워드를 3~5회 자연스럽게 배치한다. 6회 이상은 어뷰징.
4. 모든 법적 주장에는 반드시 법조문 근거를 괄호로 표기한다.
5. CTA는 반드시 구분선(━━━) 아래에 배치한다.
...
3. 핵심 키워드를 3~5회 자연스럽게 배치한다. 6회 이상은 어뷰징.
4. 모든 법적 주장에는 반드시 법조문 근거를 괄호로 표기한다.
5. CTA를 절대 넣지 않는다. "상담", "문의", "연락", "무료", "진단", admin@ 모두 금지.

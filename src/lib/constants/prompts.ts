...
const CONTENT_TYPE_RULES = `[콘텐츠 유형 판단]
주제를 분석하여 아래 5가지 유형 중 가장 적합한 것을 선택하고,
해당 유형의 구조 가이드를 따르세요.

A. 고객 사례형 — 실제 고객의 문제 해결 스토리
   구조: 훅(결과) → 고객 상황 → 문제 발견 → 해결 과정 → 결과/수치 → 시사점 → CTA
   길이: 2,000~2,500자 | 시각 자료: 2~3개 (Before/After 비교 필수)
   톤: 1인칭 스토리텔링, 감정적 몰입

B. 제도 해설형 — 제도/법률/요건을 쉽게 풀어쓰는 글
   구조: 훅(핵심 혜택) → 제도 개요 → 요건/조건 → 신청 방법 → 주의사항 → FAQ → CTA
   길이: 1,800~2,200자 | 시각 자료: 3~4개 (체크리스트 + 프로세스 + 구조도)
   톤: 친근한 해설체, "대표님" 직접 호칭

C. 뉴스 분석형 — 최신 뉴스/정책에 디딤 전문가 코멘트
   구조: 훅(뉴스 핵심) → 무슨 일인지 → 중소기업에 미치는 영향 → 디딤 코멘트 → CTA
   길이: 1,200~1,500자 | 시각 자료: 1~2개 (비교 차트 또는 타임라인)
   톤: 짧고 임팩트 있게

D. 체크리스트형 — "~하기 전 확인할 N가지"
   구조: 훅(실수 사례) → 체크 항목 1~5 (각 항목당 2~3문장) → 요약 → CTA
   길이: 1,500~1,800자 | 시각 자료: 1~2개 (체크리스트 인포그래픽 필수)
   톤: 실용적, 행동 유도

E. 비교 분석형 — 두 가지 선택지를 비교
   구조: 훅(고민 상황) → 선택지 A → 선택지 B → 비교 정리 → 추천 → CTA
   길이: 1,800~2,000자 | 시각 자료: 2~3개 (비교 차트 필수)
   톤: 객관적이되 결론은 명확히

글 시작 시 선택한 유형을 표시하지 마세요. 유형에 맞는 구조를 자연스럽게 따르기만 하면 됩니다.`;
...
const VISUAL_RULES = `[시각 자료 규칙]
■ 핵심 원칙:
- 마크다운 표(| 구분 | 내용 |)를 절대 사용하지 마세요.
  비교 데이터는 반드시 인포그래픽 이미지 마커로 표현하세요.
- 시각 자료 개수는 콘텐츠 유형에 따라 1~5개. 고정하지 마세요.
- 각 시각 자료는 본문과 다른 관점으로 정보를 재해석해야 합니다.
...
■ 이미지 설계 4원칙:
1. 임팩트 헤드라인: 이미지 상단에 한 줄 헤드라인. 이미지만 봐도 핵심 전달.
2. 관점 전환: 본문 반복 금지. Before/After, 위험/기회, 전체 조감 등 다른 각도로 재해석.
3. 감정 톤: 위기감(빨간색)/성취감(초록색)/자신감(오렌지)/긴급함(타이머) 중 선택.
4. 색상 절제: 핵심 1개만 오렌지(#D4740A), 나머지 네이비(#1B3A5C)+회색(#F5F5F5).

■ 인포그래픽 유형:
A. 비교 차트 | B. 프로세스 플로우 | C. 숫자 카드 | D. 타임라인
E. 체크리스트 | F. 퍼널 | G. 구조도 | H. 수평 막대

■ 프롬프트 필수 요소:
① "한국어 인포그래픽" ② 차트 유형 ③ 임팩트 헤드라인 ④ 감정 톤
⑤ 구체적 데이터 ⑥ 강조(오렌지 1개) ⑦ "세련된 비즈니스, 흰배경, 16:9" ⑧ 하단 출처

■ 위치 배치 원칙:
- 첫 번째: 도입부 직후 (썸네일 겸용, 핵심 메시지 요약)
- 중간: 각 소제목 뒤 (해당 섹션 핵심 데이터 시각화)
- 마지막: CTA 직전 (전체 요약 또는 Before/After)
- 연속 이미지 2개 배치 금지. 반드시 본문 단락 사이에.

■ 비교 데이터: 마크다운 표 대신 반드시 비교 차트(A) 또는 수평 막대(H) 인포그래픽으로.`;

const VISUAL_RULES_FIELD = VISUAL_RULES + `

■ 카테고리 특화: Before/After 대비(A), 절세 금액 임팩트(C), 신청 절차(B) 중심`;

const VISUAL_RULES_LOUNGE = VISUAL_RULES + `

■ 카테고리 특화: 제도 구조도(G), 타임라인(D), 트렌드 데이터(H) 중심`;

const VISUAL_RULES_DIARY = `[시각 자료 규칙]

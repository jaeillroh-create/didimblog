...
8. 이미지 마커는 반드시 아래 박스 형식 + (1) 한국어 / (2) English 이중 출력으로 작성하세요. 한 줄 압축 절대 금지. 영문 프롬프트가 누락되면 글 전체가 실패합니다.
   ...
   ━━━━━━━━━━━━━━
9. 법령 조항 번호(제○조), 시행령/시행규칙 조항 번호, 별지 서식 번호를 언급할 때 확신이 없으면 반드시 "(확인 필요)"를 병기하세요. 추정으로 번호를 기재하지 마세요. 특히 "별지 제○호 서식"은 자주 바뀌므로 확신이 없으면 서식명만 언급하거나 "(확인 필요)"로 표기하세요.
...
9. **"(확인 필요)" 표기 우회 (이것은 매우 중요)**:
   본문에 "(확인 필요)", "(확인필요)", "(미확인)", "(확정 아님)" 같은 표기가 남아 있으면
   최종 독자가 보면 안 됩니다. 다음 규칙으로 우회 표현으로 바꿔주세요.
   - 구체적 조항 번호 + (확인 필요)  →  법률명만 남기고 일반화
     · "조특법 제10조의2 (확인 필요)" → "조세특례제한법의 R&D 세액공제 규정"
     · "시행령 제8조 (확인 필요)" → "관련 시행령"
     · "별지 제○호 서식 (확인 필요)" → "관련 별지 서식 (관할 세무서/홈택스에서 최신본 확인)"
   - 단독 "(확인 필요)" → 삭제 후 문장을 자연스럽게 다듬기
   - 숫자가 불확실해서 (확인 필요) 가 붙은 경우 → "약 ~", "~ 수준", "최근 기준"
     같은 추정 표현으로 우회. 예: "공제율 25% (확인 필요)" → "공제율 약 25% 수준 (최신 기준은
     관할 세무서 확인 권장)"
   - 어떤 경우에도 최종 본문에 "(확인 필요)" 라는 글자 그대로가 남아서는 안 됩니다.
10. **팩트 불확실 우회**: 위 9번의 연장. LLM 이 정확한 조항 번호를 모르는 경우 추측해서 적지
    말고 "관련 규정", "현행 기준", "최신 개정안" 같은 일반화 표현으로 우회. 독자에게는
    "확실한 것만" 보여야 하고 추정/불확실성은 문장에 자연스럽게 녹여야 합니다.

## 수정하지 않을 것
- 글의 톤, 스토리, 논리 흐름은 변경하지 마세요
- 인포그래픽 마커 내용은 변경하지 마세요
- 법률명이나 핵심 숫자(절감액, 매출 등) 자체는 변경하지 마세요. 단, "(확인 필요)" 가 붙은
  구체적 번호/서식 번호는 위 9번에 따라 일반화하세요.
...
## 응답 형식 (JSON)
{
  ...
  "issues": [
    {
      "category": "법률팩트 | 숫자 | 단정 | 출처 | 논리",
      "severity": "심각 | 주의 | 경미",
      "original_text": "문제가 있는 원문 문장",
      "problem": "무엇이 잘못되었는지 설명",
      "suggestion": "수정 제안",
      "suggested_text": "수정된 문장 전체"
    }
  ]
}
